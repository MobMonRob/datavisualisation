 ...
 * @author rettig
 */
public class Mean_KIRK_payload_d extends AbstractAnalysis {
    
    public static void main(String[] args) throws Exception {
        AnalysisLauncher.open(new Mean_KIRK_payload_d());
    }
    ...
    public void init() throws FileNotFoundException, IOException  {
        ...
        //try {
            CSVReader csvReader = new CSVReader(new FileReader("C:\\Users\\muellersm\\Desktop\\Neuer Ordner\\d.csv"), ';' ,'"'); 
            ...
            for (int i= 1; i<records.size();i++){
                if (!records.get(i).get(129).strip().isEmpty()) {
                    try {
                        rob_meanx.add(Float.parseFloat(records.get(i).get(61)));
                    } catch (NumberFormatException e){
                        System.out.println("\""+records.get(i).get(61)+"\"");
                    }
                }
            ...
            for (int i= 1; i<records.size();i++){
                if (!records.get(i).get(107).strip().isEmpty())
                {rob_meany.add(Float.parseFloat(records.get(i).get(107)));}

            }

            List<Float> rob_meanz = new ArrayList<Float>();
            for (int i= 1; i<records.size();i++){
                if (!records.get(i).get(62).strip().isEmpty())
                {rob_meanz.add(Float.parseFloat(records.get(i).get(62)));}
            ...
            for (int i= 1; i<records.size();i++){
                if (!records.get(i).get(102).strip().isEmpty())
                {vicon_meany.add(Float.parseFloat(records.get(i).get(102)));}
                else {vicon_meany.add(0f);}
            }

            List<Float> vicon_meanz = new ArrayList<Float>();
            for (int i= 1; i<records.size();i++){
                if (!records.get(i).get(101).strip().isEmpty())
                {vicon_meanz.add(Float.parseFloat(records.get(i).get(101)));}
            ...
            for (int i= 0; i<rob_meanx.size(); i++) {
                ...
                if (radius < 1000){
                Sphere sphere = new Sphere(new Coord3d(rob_meanx.get(i),rob_meany.get(i),rob_meanz.get(i)),(float)radius/50,15,darkred);
